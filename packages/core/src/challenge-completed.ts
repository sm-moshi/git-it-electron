...
const clearStatusButton = document.getElementById(
...
verifyButton?.addEventListener("click", () => {
	...
	if (
		...
		!/Please select/.exec(directoryPathContent.innerText)
	) {
...
export function disableVerifyButtons(disabled: boolean): void {
	...
	const directoryBtn = document.getElementById(
		"select-directory",
	) as HTMLButtonElement | null;
...
export function completed(challenge: string): void {
	document.addEventListener("DOMContentLoaded", async () => {
		await checkCompletedness(challenge);
	...
	async function checkCompletedness(challenge: string): Promise<void> {
		data = await getData();
